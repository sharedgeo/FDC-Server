...
export function useFeatures(refetchProfile: () => void) {
  ...
  const saveFeatures = async (feature: { geom: object }, ticketId: number) => {
    ...
    try {
      const response = await fetch(import.meta.env.BASE_URL+"v1/features", {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${auth.user.access_token}`,
        },
        body: JSON.stringify({ feature: { geom: feature.geom, ticket_id: ticketId } }),
      });
  ...
  const deleteFeature = async (featureId: number) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.BASE_URL}v1/features/${featureId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${auth.user.access_token}`,
